...
export class ArrayColl extends KeyValueCollection {
  ...
  slice(start, end) {
    return new ArrayColl(this._array.slice(start, end));
  }
  ...
  shift() {
    let removedItem = this.array.shift();
    if (removedItem) {
      this._notifyRemoved([removedItem]);
    }
    return removedItem;
  ...
  unshift(...insertItems) {
    let length = this.array.unshift(...insertItems);
  ...
  fill(value, start, end) {
    ...
    let removedItems = this.getIndexRange(start, end - start);
    let addedItems = [];
  ...
  flat() {
    // TODO support nested ArrayColl
    return new ArrayColl(this._array.flat());
  }

  flatMap(mapFunc) {
    return new ArrayColl(this._array.flatMap(mapFunc));
  }
  ...
  reduce(func, initial) {
    ...
    return new ArrayColl(this._array.reduce(func, initial));
  }

  reduceRight(func, initial) {
    // TODO Make observable
    return new ArrayColl(this._array.reduceRight(func, initial));
  }

  /**
   * Unlike JS Array, this may not (or may) happen in-place.
   * Do not rely on either behavior.
   */
  reverse() {
    // TODO Make observable, but cannot do in-place
    let allItems = this.contents;
    this._array.reverse();
    if (allItems.length) {
      this._notifyRemoved(allItems);
      this._notifyAdded(allItems);
    }
    return this;
  }
}
